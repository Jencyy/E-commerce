...
function Cart() {
  ...
  const buyNow = async () => {
    ...
    var options = {
      key: "rzp_test_UEjeDcaHAOKjaG",
      key_secret: "vyF2Vy8bxQvXJCA7cK4q1Cx1",
  ...
  return (
    <Layout>
      <div
        className="h-screen pt-5 bg-gray-100"
      ...
      >
        ...
        <div className="mx-auto max-w-5xl px-6 md:flex md:space-x-6 xl:px-0">
          <div
            className="md:w-2/3 rounded-lg overflow-y-auto"
            style={{ maxHeight: '60vh' }} // Set max height and make it scrollable
          >
            {cartItems.map((item, index) => {
              return (
              <div
                key={index}
                  className="mb-6 rounded-lg border bg-white p-6 drop-shadow-xl sm:flex sm:justify-start"
              ...
              >
                ...
                <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                  <div className="mt-5 sm:mt-0">
                      ...
                      <h2 className="text-sm" style={{ color: mode === 'dark' ? 'white' : '' }}>{item.description}</h2>
                      <p className="mt-1 text-xs font-semibold" style={{ color: mode === 'dark' ? 'white' : '' }}>₹{item.price}</p>
                  </div>
                  <div
                    onClick={() => deleteCart(item)}
                    className="mt-4 sm:mt-0 sm:block flex justify-between sm:space-y-6 sm:space-x-6"
                  >
                    <svg
